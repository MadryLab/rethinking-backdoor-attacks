...
def main(save_path):
    ...
    solution_mmaps = {}
    for size in sizes:
        solution_path = os.path.join(save_path, 'trials', f'result_{size}.npmap')
        solution_mmaps[size] = np.lib.format.open_memmap(solution_path, mode='r').copy()

    scores = []
    for size in sizes:
        scores.append(solution_mmaps[size].sum(0, keepdims=True) / (size))

    scores = np.concatenate(scores, axis=0).sum(0)
    scores_save_path = os.path.join(save_path, 'scores', f'sample_scores.npy')
    os.makedirs(os.path.dirname(scores_save_path), exist_ok=True)
    np.save(arr=scores, file=scores_save_path)
